...
保存了总体的马尔科夫转移矩阵 对比 单个游戏搜索结果的马尔科夫转移矩阵（前向后向）
对比单个和平均马尔科夫转移矩阵的时候，是用单个的转移矩阵的值 除以 这两个字前后出现 出现的游戏的数目（不是概率）
    类似于rf-idf
这样的想法是因为游戏别名一定总会与游戏原名相伴而生，但是相伴而生的还有大量的“下载” “硬盘” “中文版”等等的词汇
因为游戏名不好分词，一来是分词可能切开该有的游戏名，与其他无关的字连在一起。
                二来是分词会扩大markov的空间，似的统计更加稀疏，可能会影响效果。
"""
import pickle
import re
import string
